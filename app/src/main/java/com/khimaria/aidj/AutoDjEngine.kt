...
object AutoDjEngine {
    ...
    fun rank(
        ...
        val sessionHistory = history.filter { it.observedAtMs > 0L && now - it.observedAtMs <= SESSION_GAP_MS }
    ...
    private fun scoreTrack(
        ...
        current?.releaseYear?.let { currentYear ->
            track.releaseYear?.let { candidateYear ->
                val diff = abs(currentYear - candidateYear)
                metadataSignals += when {
                    diff <= 2 -> 95.0
                    diff <= 5 -> 85.0
                    diff <= 10 -> 72.0
                    diff <= 20 -> 56.0
                ...
                if (diff <= 5) reasons += "dönem uyumu"
            }
        }
        current?.durationMs?.let { currentDuration ->
            track.durationMs?.let { candidateDuration ->
                if (currentDuration > 0 && candidateDuration > 0) {
                    val ratio = abs(currentDuration - candidateDuration).toDouble() / currentDuration.toDouble()
                    metadataSignals += (100.0 - ratio * 80.0).coerceIn(35.0, 100.0)
                }
            }
        }
        current?.bpm?.let { currentBpm ->
            track.bpm?.let { candidateBpm ->
                val d = bpmDistance(currentBpm, candidateBpm)
                metadataSignals += (100.0 - d * 3.2).coerceIn(20.0, 100.0)
                if (d <= 5.0) reasons += "BPM uyumu"
            }
        }
        current?.energy?.let { currentEnergy ->
            track.energy?.let { candidateEnergy ->
                val d = abs(currentEnergy - candidateEnergy)
                metadataSignals += (100.0 - d * 100.0).coerceIn(20.0, 100.0)
                if (d <= .12) reasons += "enerji uyumu"
            }
        }
        current?.musicalKey?.takeIf { it.isNotBlank() }?.let { currentKey ->
            track.musicalKey?.takeIf { it.isNotBlank() }?.let { candidateKey ->
                metadataSignals += if (currentKey.equals(candidateKey, true)) 100.0 else 58.0
                if (currentKey.equals(candidateKey, true)) reasons += "tonal uyum"
            }
        }
        ...
        val popularityFit = track.popularity?.let { p ->
            when {
                p in 25..90 -> 78.0
                p > 90 -> if (mode == FlowMode.DISCOVERY) 58.0 else 72.0
                else -> if (mode == FlowMode.SAFE) 45.0 else 62.0
            }
        } ?: 55.0
    ...
    private fun bpmDistance(a: Double, b: Double): Double {
        val candidates = listOf(abs(a - b), abs(a - b * 2.0), abs(a * 2.0 - b))
        return candidates.minOrNull() ?: abs(a - b)
    }
