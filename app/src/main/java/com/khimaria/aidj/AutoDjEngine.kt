...
object AutoDjEngine {
    ...
    private fun scoreTrack(
        ...
        val currentArtist = current?.artist?.lowercase().orEmpty()
        ...
        val relation = when {
            ...
            track.source.startsWith("itunes:artist") && sameArtist -> 48.0
            track.source.startsWith("itunes:artist") -> 66.0
            track.source.startsWith("itunes:genre") -> 60.0
            track.source.startsWith("musicbrainz:") && sameArtist -> 46.0
            track.source.startsWith("musicbrainz:") -> 56.0
        ...
        val genreFit = when {
            seedTags.isEmpty() || trackTags.isEmpty() -> 52.0
            overlap.isNotEmpty() -> (74 + overlap.size * 7).coerceAtMost(96).toDouble()
            else -> 30.0
        }
        ...
        if (artistKey in likedArtists) { tasteFit += 18; reasons += "sevdiğin sanatçı çizgisi" }
        if (artistKey in dislikedArtists) tasteFit -= 34
        ...
        if (sessionHistory.isEmpty()) sessionFit = if (sameArtist) 64.0 else 72.0
        ...
        current?.releaseYear?.let { cy -> track.releaseYear?.let { y ->
            val d = abs(cy - y)
            metadataSignals += when { d <= 2 -> 95.0; d <= 5 -> 85.0; d <= 10 -> 72.0; d <= 20 -> 56.0; else -> 42.0 }
            if (d <= 5) reasons += "dönem uyumu"
        } }
        current?.durationMs?.let { cd -> track.durationMs?.let { d ->
            if (cd > 0 && d > 0) metadataSignals += (100.0 - abs(cd - d).toDouble() / cd * 80.0).coerceIn(35.0, 100.0)
        } }
        current?.bpm?.let { cb -> track.bpm?.let { b ->
            val d = bpmDistance(cb, b)
            metadataSignals += (100.0 - d * 3.2).coerceIn(20.0, 100.0)
            if (d <= 5.0) reasons += "BPM uyumu"
        } }
        current?.energy?.let { ce -> track.energy?.let { e ->
            val d = abs(ce - e)
            metadataSignals += (100.0 - d * 100.0).coerceIn(20.0, 100.0)
            if (d <= .12) reasons += "enerji uyumu"
        } }
        current?.musicalKey?.takeIf { it.isNotBlank() }?.let { ck ->
            track.musicalKey?.takeIf { it.isNotBlank() }?.let { k ->
                metadataSignals += if (ck.equals(k, true)) 100.0 else 58.0
                if (ck.equals(k, true)) reasons += "tonal uyum"
            }
        }
        val metadataFit = if (metadataSignals.isEmpty()) 50.0 else metadataSignals.average()
        val popularityFit = track.popularity?.let { p ->
            when { p in 25..90 -> 78.0; p > 90 -> if (mode == FlowMode.DISCOVERY) 58.0 else 72.0; else -> if (mode == FlowMode.SAFE) 45.0 else 62.0 }
        } ?: 52.0

        val weighted = relation * .24 + genreFit * .20 + tasteFit * .16 + sessionFit * .16 + noveltyFit * .09 + metadataFit * .10 + popularityFit * .05
        val score = weighted.toInt().coerceIn(0, 100)

        var confidence = 32 + when { relation >= 84 -> 18; relation >= 66 -> 11; else -> 5 }
        ...
        if (sameArtist && relation < 80 && metadataSignals.isEmpty()) confidence -= 10
        confidence = confidence.coerceIn(20, 96)
    ...
    private fun normalizeTag(raw: String): String = raw.lowercase().trim()
        .replace("alternative rock", "rock")
        .replace("turkish rock", "rock")
        .replace("indie rock", "rock")
        .replace("pop/rock", "rock")
